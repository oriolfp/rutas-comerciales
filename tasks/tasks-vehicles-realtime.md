...
- [ ] 0.0 Create feature branch
  - [ ] 0.1 `git checkout -b feature/vehicles-realtime` des de `main`
  - [ ] 0.2 Primer push per establir tracking remot

- [ ] 1.0 Backend: endpoint /api/vehicles
  - [ ] 1.1 Estendre `fetchIBus` (a `_tmb.ts`) amb una variant `fetchIBusBatch(liniaCodi, paradaCodis[])` que faci `mapLimit(stops, 10, fetchIBus(..., all=true))`. Retorna `{ paradaCodi, arribades[] }[]`.
  - [ ] 1.2 Estendre `fetchIMetro` (a `_tmb.ts`) per acceptar múltiples codis d'estació en una sola crida (TMB `?estacions=CSV` ja suporta CSV; cal canviar el tipus i la lògica de l'iterador).
  - [ ] 1.3 Crear `netlify/functions/vehicles.ts` que rep path `{tipus}/{liniaCodi}`, carrega les parades de la línia (via `fetchParades`), crida el batch corresponent i agrega.
  - [ ] 1.4 Implementar dedupe: per a metro, agrupar per `codi_servei`; per a bus, agrupar per `routeId`. Si manca id, agrupar per `destinacio` i quedar-se amb la parada de minuts mínims.
  - [ ] 1.5 Per a cada vehicle, calcular la **cua** de parades pendents: filtrar parades del seu sentit (que matchin `destinacio`), agafar les ≤ 4 properes amb temps creixent.
  - [ ] 1.6 Retornar shape:
    ```ts
    {
    ...
    ```
  - [ ] 1.7 Headers `cache-control: public, max-age=30` + `netlify-cdn-cache-control: public, max-age=30, durable`.
  - [ ] 1.8 Afegir redirect a `netlify.toml`: `from = "/api/vehicles/*"` → `to = "/.netlify/functions/vehicles?path=:splat"`.
  - [ ] 1.9 Manejo d'errors: si TMB respon 401/403, retornar 200 amb `vehicles: []` i camp `missatge`.

- [ ] 2.0 Frontend data layer
  - [ ] 2.1 Afegir tipus a `src/types/tmb.ts`:
    - `VehicleRaw` (resposta del backend, shape de 1.6)
    - `VehiclePos` (estesa amb `lat`, `lng`, `direccio: 'left' | 'right'` un cop interpolada)
  - [ ] 2.2 Afegir `getVehicles(tipus, liniaCodi): Promise<VehiclesResposta>` a `src/services/tmb.ts`
  - [ ] 2.3 Crear `src/hooks/useVehicles.ts` que rep `{ tipus, liniaId, enabled }` i exposa `{ data, loading, error, refresh, lastFetchedAt }`. **No auto-refresh** intern: només es refresca via `refresh()` o quan canvia la línia.
  - [ ] 2.4 Tests bàsics del hook: carrega inicial, refresh manual, reset en canvi de línia.

- [ ] 3.0 Interpolació de posició
  - [ ] 3.1 Crear `src/utils/transit.ts` amb `SPEED_M_S = { bus: 5, metro: 8 }` i `directionOfSegment([a, b]): 'left' | 'right'` (compara `b.lng - a.lng`).
  - [ ] 3.2 Crear `src/utils/route.ts` amb:
  ...
  - [ ] 3.3 `extrapolateVehiclePosition({ polyline, parades, properaParadaCodi, minutsFinsProperaParada, speedMS })`:
  ...
  - [ ] 3.4 Tests amb una polilínia sintètica recta (10 punts), parada coneguda i temps de 1, 5, 100 min → posicions esperades.
  - [ ] 3.5 Tests amb temps molt grans (vehicle "abans del principi"): clamp a `distance = 0` correcte.

- [ ] 4.0 Marcadors de vehicle al mapa
  - [ ] 4.1 Crear `src/components/VehicleMarker.tsx` que rep `{ vehicle: VehiclePos, color: string, tipus: TransportType, liniaCodi: string }`.
  - [ ] 4.2 Renderitzar amb Leaflet `Marker` + `divIcon({ html: ReactDOMServer.renderToStaticMarkup(<svg>...</svg>), className: 'vehicle-icon-wrapper', iconSize: [...], iconAnchor: [...] })`. Inline SVG amb el color de la línia.
  - [ ] 4.3 Variant Bus (codi cercat al mockup v3 "Bus variant C"): rect simètric + chevron `>` separat del davant. Flip via CSS `transform: scaleX(-1)` quan `direccio === 'left'`.
  - [ ] 4.4 Variant Metro: silueta arrodonida amb nas + llum frontal. Flip horitzontal idèntic.
  - [ ] 4.5 `Tooltip` mostrant `{liniaCodi} · {destinacio}` al hover/long-press.
  - [ ] 4.6 `Popup` amb:
    - Capçalera: badge de línia + destinació
    - Llista de `cuaProperesParades` (fins a 4 ítems) amb minuts a la dreta en vermell
  - [ ] 4.7 Estils CSS al `src/App.css` per a `.vehicle-icon-wrapper`, `.vehicle-popup`, `.vehicle-queue-item`.

- [ ] 5.0 Botó de refresc + cooldown
  - [ ] 5.1 Crear `src/hooks/useCooldown.ts`:
  ...
  - [ ] 5.2 Tests amb fake timers (`vi.useFakeTimers`).
  - [ ] 5.3 Crear `src/components/RefreshControl.tsx`:
  ...
  - [ ] 5.4 Prop `onRefresh: () => Promise<void>`. En clicar: passa a `loading`, després `await onRefresh()`, després si OK → `cooldown(2 min)`; si KO → torna a `idle` i mostra toast d'error.
  - [ ] 5.5 Estils al `App.css` per al botó i la rotació de loading.

- [ ] 6.0 Integració, tests i PR
  - [ ] 6.1 Integrar a `MapView.tsx`: capa de `<VehicleMarker>` per a cada `vehiclesAmbPos` només quan hi ha línia seleccionada amb polilínia carregada. Renderitzar `<RefreshControl>` al mateix `MapView` només si la línia està seleccionada.
  - [ ] 6.2 A `LiniesView.tsx`:
  ...
  - [ ] 6.3 Reset del cooldown global si l'usuari fa toggle a "Aprop meu" — opcional, decidir si volem persistir o no. Per ara: persisteix entre canvis de línia, es reseteja en sortir del mode "Línies".
  - [ ] 6.4 Tests addicionals:
  ...
  - [ ] 6.6 Actualitzar el mockup v3 (si cal després de prova real) i fer commit.
  - [ ] 6.7 Obrir PR `feature/vehicles-realtime` → `main` amb captures i checklist de test plan manual:
